    ...
    async function (result) {
      if (result.length > 0) {
        ...
        if (match) {
          ...
          return res.status(200).json({ status: false, message: 'Wrong password. The password you entered does not match.', client: {} });
        }
      } else {
        return res.status(200).json({ status: false, message: 'No User with this number', client: {} });
      }
    },
    function (error) {
      console.log(error);
      return res.status(503).json({status:false,message:'Internal Server Error',client:{}})
    }
  );
};
...
exports.numberVerify = (req, res) => {
  ...
  db.query("SELECT * FROM `user` WHERE `phone` = ?", [number],
  (err1, res1)=>{
    if (!err1) {
      if (res1.length > 0) {
        res.send("success")
...
exports.passReset = (req, res) => {
  ...
  console.log(oldPassword, newPassword, numb)
